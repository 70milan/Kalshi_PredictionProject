...
def generate_mispricing_scores(spark, kalshi_history_path, gdelt_summaries_path, news_summaries_path, mapping_file):
    ...
    # 5. Calculate Mispricing Score (V2 — validated against a forward-outcome backtest)
    # The OLD formula multiplied by (1 - max_abs_delta), PENALIZING markets that had already
    # moved. A forward-window backtest proved this backwards: high-score markets moved LESS
    # than low-score controls (0.63x at 24h) because price stability is autocorrelated —
    # "hasn't moved" predicts "won't move", not "about to move". V2 instead REWARDS recent
    # volatility, which empirically predicts continued movement and lifts the directional
    # edge from ~57% (24h) toward ~68% (14d).
    #
    #   raw_v2 = spike × |sentiment| × (1 + |delta_24h|)
    #
    # raw_v2 is fat-tailed (p95 ≈ 306, max ≈ 22000). The old ×20+cap-100 saturated ~22% of
    # markets at exactly 100, so "score >= 80" admitted ~24% of ALL markets — barely a filter.
    # We log-normalize and ANCHOR the validated top-5% boundary (raw_v2 ≈ 306) to exactly 80,
    ...
    raw_v2 = (
        F.when(F.col("max_spike_multiplier").isNull(), F.lit(0.0))
         .otherwise(
             F.col("max_spike_multiplier") * F.abs(F.col("avg_sentiment")) *
             (F.lit(1.0) + F.abs(F.col("delta_24h")))
         )
    )
    df_final = df_final.withColumn("raw_score", raw_v2)

    # Log-anchored normalization: score = 80 * ln(1+raw) / ln(1+anchor), clamped [0,100].
